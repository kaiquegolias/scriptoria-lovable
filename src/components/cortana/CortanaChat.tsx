...
import { Send, Trash2, StopCircle, Bot, User, Sparkles, Zap } from 'lucide-react';
    ...
    <div className="flex flex-col h-[calc(100vh-12rem)] max-h-[780px]">
      ...
      <div className="flex-1 overflow-hidden rounded-2xl border border-border/40 bg-card/50 backdrop-blur-sm">
        <ScrollArea className="h-full p-5" ref={scrollRef}>
          {messages.length === 0 ? (
            <WelcomeScreen onQuickPrompt={(p) => sendMessage(p)} />
          ) : (
            <div className="space-y-5">
              ...
              {isLoading && messages[messages.length - 1]?.role !== 'assistant' && (
                <div className="flex items-center gap-3 pl-12">
                  ...
                  <span className="text-sm text-muted-foreground">Cortana está pensando...</span>
                </div>
              )}
            </div>
          )}
      ...
      <div className="mt-3 flex gap-2 items-end">
        <div className="flex-1 relative">
          <Textarea
            ...
            placeholder="Pergunte à Cortana..."
        ...
        <div className="flex gap-1.5">
          {isLoading ? (
            <Button size="icon" variant="destructive" onClick={stopGeneration} className="h-[52px] w-[52px] rounded-xl shadow-sm">
          ...
          {messages.length > 0 && (
            <Button size="icon" variant="outline" onClick={clearChat} className="h-[52px] w-[52px] rounded-xl text-muted-foreground">
...
const WelcomeScreen: React.FC<{ onQuickPrompt: (p: string) => void }> = ({ onQuickPrompt }) => (
  <div className="flex flex-col items-center justify-center h-full text-center py-16 px-6">
    ...
    <h3 className="text-2xl font-bold mb-2">Olá! Eu sou a Cortana 👋</h3>
    <p className="text-muted-foreground text-sm max-w-md mb-8 leading-relaxed">
      Sua assistente de IA com personalidade. Pergunte sobre erros, soluções, scripts e chamados da sua base de conhecimento.
    </p>
